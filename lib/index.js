...
(function (angular) {
  ...
  var db = function db(key) {
    return preprocess(require('../database/' + key + '.json'));
  };

  var resolveResultbyField = function resolveResultbyField(type, searchStr, maxResult) {
    var dbKey = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : 'db';
    ...
    var possibles = [];
    try {
      possibles = db(dbKey).filter(function (item) {
  ...
  var searchAddressByDistrict = function searchAddressByDistrict(searchStr, maxResult, dbKey) {
    return resolveResultbyField('district', searchStr, maxResult, dbKey);
  };
  var searchAddressByAmphoe = function searchAddressByAmphoe(searchStr, maxResult, dbKey) {
    return resolveResultbyField('amphoe', searchStr, maxResult, dbKey);
  };
  var searchAddressByProvince = function searchAddressByProvince(searchStr, maxResult, dbKey) {
    return resolveResultbyField('province', searchStr, maxResult, dbKey);
  };
  var searchAddressByZipcode = function searchAddressByZipcode(searchStr, maxResult, dbKey) {
    return resolveResultbyField('zipcode', searchStr, maxResult, dbKey);
  };
