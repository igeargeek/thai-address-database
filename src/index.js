 ...
 */
  ...
  const db = (key) => preprocess(require(`../database/${key}.json`))

  const resolveResultbyField = (type, searchStr, maxResult, dbKey = 'db') => {
    ...
    let possibles = []
    try {
      possibles = db(dbKey).filter(item => {
  ...
  const searchAddressByDistrict = (searchStr, maxResult, dbKey) => {
    return resolveResultbyField('district', searchStr, maxResult, dbKey)
  }
  const searchAddressByAmphoe = (searchStr, maxResult, dbKey) => {
    return resolveResultbyField('amphoe', searchStr, maxResult, dbKey)
  }
  const searchAddressByProvince = (searchStr, maxResult, dbKey) => {
    return resolveResultbyField('province', searchStr, maxResult, dbKey)
  }
  const searchAddressByZipcode = (searchStr, maxResult, dbKey) => {
